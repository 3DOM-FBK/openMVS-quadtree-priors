...
struct BlockWeightedPatchFix {
	struct Pixel {
		float weight;
		float tempWeight;
	};
	std::vector<Pixel> weights;
	float sumWeights;
	float normSq0;

	BlockWeightedPatchFix(int size) : normSq0(0) 
	{
		weights.resize(size);
	}
};
...
struct MVS_API DepthData {
	...
	Image8U3 labels; //change accordingly to Image8U or 8U3 according the labels
...
struct MVS_API DepthEstimator {
	...
	struct QuadTreeBlock {
		...
		float conf;
		int level;
		int bSizeHalfWindowWidth;
		int bSizeHalfWindowHeight;
		int bTexels;
		int color;
		bool assigned;
		int numPoints;
		...
		QuadTreeBlock() 
		{
			bSizeHalfWindowWidth = nSizeHalfWindow;
			bSizeHalfWindowHeight = nSizeHalfWindow;
		...
		QuadTreeBlock(int _id, ImageRef _center, int _width, int _height, int _level, int _color) :
			id(_id), center(_center), width(_width), height(_height), level(_level), color(_color)
		{
			//hist = cv::Mat();

			bSizeHalfWindowWidth = width / 2 + nSizeHalfWindow;// +nSizeHalfWindow);
			bSizeHalfWindowHeight = height / 2 + nSizeHalfWindow;// +nSizeHalfWindow);

			// Ensure that half window size is an odd number
			if (bSizeHalfWindowWidth % 2 == 0)
				bSizeHalfWindowWidth++;

			if (bSizeHalfWindowHeight % 2 == 0)
				bSizeHalfWindowHeight++;

			// Take the max half win size between the 2, ensure that the block is square
			if (bSizeHalfWindowWidth > bSizeHalfWindowHeight)
				bSizeHalfWindowHeight = bSizeHalfWindowWidth;
			else
				bSizeHalfWindowWidth = bSizeHalfWindowHeight;

			bTexels = ((bSizeHalfWindowWidth * 2 + nSizeStep) / nSizeStep * (bSizeHalfWindowHeight * 2 + nSizeStep) / nSizeStep) * TexelChannels;
	...
	typedef CLISTDEF0(QuadTreeBlock) QuadTreeBlockArr;
