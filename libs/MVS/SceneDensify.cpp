...
{	
	...
	if (OPTDENSE::nUseSemantic)
	{
		...
		for (unsigned iter = OPTDENSE::nEstimationIters; iter < OPTDENSE::nEstimationIters+4; ++iter) {
			// create working threads
			idxPixel = -1;
			ASSERT(estimators.IsEmpty());
			while (estimators.GetSize() < nMaxThreads)
...
bool DepthMapsData::GenerateDepthPrior(DepthData& depthData, DepthEstimator::MapRefArr& coords)
{
	...
	std::cout << "\nGenerating Priors... " << std::endl;

	depthData.images.First().depthMapPrior = DepthMap(depthData.GetView().image.size());
	DepthMap& depthMapPrior = depthData.images.First().depthMapPrior;
	...
	//Quadtree computation
	FBK3DOM::QuadTree quadTree(depthData.GetView().pImageData->image, 3, 1);

	Image8U3 tree;
	Image8U3 cells;

	depthData.GetView().pImageData->image.copyTo(tree);
	DepthEstimator::QuadTreeBlockArr blocks;	

	for (auto it = quadTree.nodes.begin(); it != quadTree.nodes.end(); it++)
	{
		auto node = *it;		

		DepthMap cellDepth = depthData.depthMap(cv::Rect(node->x, node->y, node->width, node->height));
		NormalMap cellNormal = depthData.normalMap(cv::Rect(node->x, node->y, node->width, node->height));

		cv::Scalar depth = cv::mean(cellDepth);
		cv::Scalar n = cv::mean(cellNormal);

		DepthEstimator::QuadTreeBlock quadTreeBlock = DepthEstimator::QuadTreeBlock(node->id, ImageRef(node->x + node->width / 2, node->y + node->height / 2), node->width, node->height, node->level, node->color);
		quadTreeBlock.depth = depth.val[0];
		quadTreeBlock.normal = Normal(n.val[0], n.val[1], n.val[2]);
		quadTreeBlock.avgColor = node->avgColor;
		blocks.Insert(quadTreeBlock);

		cv::rectangle(tree, cv::Rect(node->x, node->y, node->width, node->height), cv::Vec3b(0, 255, 255), 1);

	}

	std::cout << "*** quadTree.max_level: " << quadTree.max_level << std::endl;
	cv::imwrite(ComposeDepthFilePath(depthData.GetView().GetID(), "quadtree.png"), tree);

	ImageRefArr regionPixels; // pixels of the region planar area 			

	std::vector<CGALPointWithNormal> ransacPointSamples;	

	unsigned int regionIndex = 1;	

	IDX idx = -1;
	while (++idx < coords.GetSize())
	{
		const ImageRef& coord = coords[idx];
		depthMapPrior(coord) = 0;
		normalMap(coord) = Normal::ZERO;
	
		if (depthData.depthMap(coord) != 0 && depthData.depthMap(coord) < 20 && depthData.depthMap(coord) > 0.5 && depthData.confMap(coord) < 0.20)
		{
	...
	std::cout << "ransacPointSamples: " << ransacPointSamples.size() << std::endl;

	const int nb_neighbors = 20; // considers n nearest neighbor points, before was 6, the bigger the slower

	// Estimate scale of the point set with average spacing
	double average_spacing = CGAL::compute_average_spacing<CGAL::Sequential_tag>(ransacPointSamples, nb_neighbors, CGAL::parameters::point_map(CGAL::First_of_pair_property_map<CGALPointWithNormal>()));

	ransacPointSamples.erase(
	...
	std::cout << "...after filtering: " << ransacPointSamples.size() << std::endl;
	...
	Local_eigen_analysis eigen = Local_eigen_analysis::create_from_point_set(ransacPointSamples, CGAL::First_of_pair_property_map<CGALPointWithNormal>(), neighborhood.k_neighbor_query(60));

	for (size_t i = 0; i < ransacPointSamples.size(); i++)
	{				
	...
	std::cout << "...after planarity: " << filteredPointSamples.size() << std::endl;
	
	// Recompute average spacing after planarity filtering.
	...
	parameters.cluster_epsilon = average_spacing;
	// Set maximum normal deviation (rad).
	parameters.normal_threshold = 0.92;
	...
	Efficient_ransac::Plane_range shapes = ransac.planes();

	Efficient_ransac::Plane_range::iterator it = shapes.begin();
	...
	while (it != shapes.end())
	{
		DepthEstimator::QuadTreeBlockArr assigned_blocks;
		DepthEstimator::QuadTreeBlockArr unassigned_blocks;

		cv::Mat mask = cv::Mat::zeros(depthData.depthMap.rows, depthData.depthMap.cols, CV_8UC1);
		...
		{								
			...
			points_with_normals.push_back(CGALPointWithNormal(CGALPoint(filteredPointSamples[(*index)].first.x(), filteredPointSamples[(*index)].first.y(), filteredPointSamples[(*index)].first.z()),
				CGALNormal(filteredPointSamples[(*index)].second.x(), filteredPointSamples[(*index)].second.y(), filteredPointSamples[(*index)].second.z())));
		...
		Normal planenormal(finalPlane.m_vN.x(), finalPlane.m_vN.y(), finalPlane.m_vN.z()); // Normal in object space	

		if (plane.has_on_positive_side(CGALPoint(depthData.images.First().camera.C.x, depthData.images.First().camera.C.y, depthData.images.First().camera.C.z))==1)
		{
		...
		cv::Mat rgb = cv::Mat::zeros(depthData.GetView().image.size(), CV_8UC3);
		cv::Mat rgb_blocks = cv::Mat::zeros(depthData.GetView().image.size(), CV_8UC3);

		for (std::vector<std::size_t>::const_iterator index = (*it)->indices_of_assigned_points().begin(); index != (*it)->indices_of_assigned_points().end(); index++)
		{								
			ImageRef seed = depthData.images.First().camera.TransformPointW2I(Point3(filteredPointSamples[(*index)].first.x(), filteredPointSamples[(*index)].first.y(), filteredPointSamples[(*index)].first.z()));					

			int block_id = quadTree.mask(seed);

			blocks[block_id].numPoints++;
		}

		int count = 0;
		// iterate through seeds again and see in which quadtree block they belong
		for (std::vector<std::size_t>::const_iterator index = (*it)->indices_of_assigned_points().begin(); index != (*it)->indices_of_assigned_points().end(); index++)
		{								
			ImageRef seed = depthData.images.First().camera.TransformPointW2I(Point3(filteredPointSamples[(*index)].first.x(), filteredPointSamples[(*index)].first.y(), filteredPointSamples[(*index)].first.z()));					
			...
			rgb.at<cv::Vec3b>(seed) = cv::Vec3b(color, 0, 0);

			Point3d grid(depthData.images.First().camera.TransformPointI2W(Point3(seed.x, seed.y, 1)));
			Point3d direction = grid - origin;
			const Ray3d ray(origin, direction);					
			const Point3d intersection(ray.Intersects(finalPlane));					

			int block_id = quadTree.mask(seed);
			DepthEstimator::QuadTreeBlock& b = blocks[block_id];
			
			std::stack<int> blocks_stack;

			// start from small size quadtrees with enough assigned points
			if (b.level > quadTree.max_level-4 && b.level < quadTree.max_level-1 && b.numPoints > 2)
			{
				blocks_stack.push(b.id);
			}

			//assign depth and normal to the center of the block				
			while (!blocks_stack.empty())
			{
				int current_id = blocks_stack.top();
				blocks_stack.pop();

				if (blocks[current_id].assigned == false) 
				{
					blocks[current_id].assigned = true;
					assigned_blocks.push_back(blocks[current_id]);

					Point3d grid_center(depthData.images.First().camera.TransformPointI2W(Point3(blocks[current_id].center.x, blocks[current_id].center.y, 1)));
					Point3d direction_center = grid_center - origin;
					//depth of the center of the block with ray traycing
					const Ray3 ray_center(origin, direction_center);
					const Point3d intersection_center(ray_center.Intersects(finalPlane));	
					depthMapPrior(blocks[current_id].center) = depthData.images.First().camera.TransformPointW2I3(intersection_center).z;
					blocks[current_id].depth = depthMapPrior(blocks[current_id].center);				
					
					std::vector<std::shared_ptr<FBK3DOM::QuadTreeNode>> neighboursList;
					
					quadTree.neighboursLeft(quadTree.nodes[current_id], neighboursList);
					quadTree.neighboursTop(quadTree.nodes[current_id], neighboursList);
					quadTree.neighboursRight(quadTree.nodes[current_id], neighboursList);
					quadTree.neighboursBottom(quadTree.nodes[current_id], neighboursList);
					
					const cv::Rect rect_block(blocks[current_id].center.x-(blocks[current_id].width/2), blocks[current_id].center.y-(blocks[current_id].height/2), blocks[current_id].width, blocks[current_id].height);
					cv::Mat crop_block = quadTree.image(rect_block);
					cv::Scalar mean_block;
					cv::Scalar stddev_block;
					cv::meanStdDev(crop_block, mean_block, stddev_block);
				
					quadTree.nodes[current_id]->avgColor;
					blocks[current_id].avgColor;

					for (auto it = neighboursList.begin(); it != neighboursList.end(); it++)
					{
						auto neighbourNode = *it;
						
						DepthEstimator::QuadTreeBlock& neighbourBlock = blocks[neighbourNode->id];
						
						if (neighbourBlock.assigned == false) {

							const cv::Rect rect_neighbour(neighbourBlock.center.x-(neighbourBlock.width/2), neighbourBlock.center.y-(neighbourBlock.height/2), neighbourBlock.width, neighbourBlock.height);
															
							cv::Mat crop_neigh = quadTree.image(rect_neighbour);
							cv::Scalar mean_neigh;
							cv::Scalar stddev_neigh;
							cv::meanStdDev(crop_neigh, mean_neigh, stddev_neigh);	
						
							double diff_1 = abs(mean_neigh[1]-mean_block[1]);
							double diff_2 = abs(mean_neigh[2]-mean_block[2]);

							//propagate teighboring blocks only if they are of similar color and big size, so that it doesn't cross the edges
							if (diff_1 < 1.5 && diff_2 < 1.5 && neighbourBlock.level < quadTree.max_level-1)
							{	
								blocks_stack.push(neighbourBlock.id);
							}	

							else
							{
								unassigned_blocks.push_back(neighbourBlock);
							}							
						}
					}
				}
		...
		// assign depth and normal prior to all pixels of each block
		Depth max_depth = depthData.dMax;
		
		for (auto it = assigned_blocks.begin(); it != assigned_blocks.end(); it++)
		{
			auto node = *it;

			for (int x = node.center.x - node.width/2; x <= node.center.x + node.width/2; x++)
			{
				for (int y = node.center.y - node.height/2; y <= node.center.y + node.height/2; y++)
				{
					ImageRef pixel(x, y);
					Point3d grid_block(depthData.images.First().camera.TransformPointI2W(Point3(pixel.x, pixel.y, 1)));
					Point3d direction_block = grid_block - origin;

					if (depthData.depthMap.isInside(pixel))
					{							
						Depth depthNew = node.depth;
						Normal normalNew = node.normal;

						int color = 255;

						rgb_blocks.at<cv::Vec3b>(pixel) = cv::Vec3b(color, color, 0);

						if (depthNew != 0) 
						{
							const Ray3 ray_block(origin, direction_block);
							const Point3d intersection_block(ray_block.Intersects(finalPlane));	
							depthNew = depthData.images.First().camera.TransformPointW2I3(intersection_block).z;
							normalNew = depthData.images.First().camera.R * Cast<REAL>(planenormal);
							
							// adjust normal such that it makes at most 90 degrees with the viewing angle
							const Normal viewDir = (Point3d)(pixel) - origin;
							
							const float dotproduct(normalNew.dot(viewDir));
							if (dotproduct <= 0){
								normalNew = RMatrixBaseF(normalNew.cross(viewDir), MINF((ACOS(dotproduct/norm(viewDir))-FD2R(90.f))*1.01f, -0.001f)) * normalNew;
							}

						}
						
						// filter out outlier depth values
						if (depthNew > 40)
						{
							depthNew = 0;
						}

						depthMapPrior(pixel) = depthNew;
						// planenormal is in the object space, we need to calculate the normal in the image space
						normalMap(pixel) = normalNew;
					}
				}
			}
		}			
	...
	#if 1 && TD_VERBOSE != TD_VERBOSE_OFF
	if (g_nVerbosityLevel > 0) 
	{
		ExportDepthMap(ComposeDepthFilePath(depthData.GetView().GetID(), "prior.png"), depthMapPrior);
		SaveDepthMap(ComposeDepthFilePath(depthData.GetView().GetID(), "prior.dmap"), depthMapPrior);
		ExportNormalMap(ComposeDepthFilePath(depthData.GetView().GetID(), "normal.prior.png"), normalMap);
		ExportPointCloud(ComposeDepthFilePath(depthData.GetView().GetID(), "prior.ply"), *depthData.images.First().pImageData, depthMapPrior, normalMap);		
		std:: cout << "time: " << TD_TIMER_GET_FMT().c_str();
	}
	#endif
	...
	while (true) {
		...
		case EVT_SAVEDEPTHMAP: {
			...
			if (g_nVerbosityLevel > 2) {
				...
				//ExportDepthMapRaw(ComposeDepthFilePath(depthData.GetView().GetID(), "raw_16bit.png"), depthData.depthMap);
				if (g_nVerbosityLevel > 4) {	
